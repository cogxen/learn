export default [
  {
    text: "Foundations",
    items: [
      {
        text: "Overview",
        link: "/lectures/data-science/foundations/",
      },
    ],
  },
  {
    text: "Roles and Impact",
    items: [
      {
        text: "Data Scientist",
        link: "/lectures/data-science/foundations/roles-and-impact/data-scientist",
      },
      {
        text: "Data Analyst",
        link: "/lectures/data-science/foundations/roles-and-impact/data-analyst",
      },
      {
        text: "Data Engineer",
        link: "/lectures/data-science/foundations/roles-and-impact/data-engineer",
      },
      {
        text: "Machine Learning Engineer",
        link: "/lectures/data-science/foundations/roles-and-impact/machine-learning-engineer",
      },
    ],
  },
  {
    text: "Power of Data Science",
    items: [
      {
        text: "In Business",
        link: "/lectures/data-science/foundations/power-of-data-science/in-business",
      },
      {
        text: "In Healthcare",
        link: "/lectures/data-science/foundations/power-of-data-science/in-healthcare",
      },
      {
        text: "In Government",
        link: "/lectures/data-science/foundations/power-of-data-science/in-government",
      },
      {
        text: "In Education",
        link: "/lectures/data-science/foundations/power-of-data-science/in-education",
      },
      {
        text: "In Energy",
        link: "/lectures/data-science/foundations/power-of-data-science/in-energy",
      },
      {
        text: "In Sports",
        link: "/lectures/data-science/foundations/power-of-data-science/in-sports",
      },
      {
        text: "In Manufacturing",
        link: "/lectures/data-science/foundations/power-of-data-science/in-manufacturing",
      },
      {
        text: "In Agriculture",
        link: "/lectures/data-science/foundations/power-of-data-science/in-agriculture",
      },
      {
        text: "In Cybersecurity",
        link: "/lectures/data-science/foundations/power-of-data-science/in-cybersecurity",
      },
    ],
  },
  {
    text: "Life Cycle",
    items: [
      {
        text: "Data Collection",
        link: "/lectures/data-science/foundations/data-collection",
      },
      {
        text: "Data Cleaning",
        link: "/lectures/data-science/foundations/data-cleaning",
      },
      {
        text: "Data Analysis",
        link: "/lectures/data-science/foundations/data-analysis",
      },
      {
        text: "Model Building",
        link: "/lectures/data-science/foundations/model-building",
      },
      {
        text: "Model Deployment",
        link: "/lectures/data-science/foundations/model-deployment",
      },
    ],
  },
  {
    text: "Work of a Data Scientist",
    items: [
      {
        text: "Day-to-Day Responsibilities",
        link: "/lectures/data-science/foundations/day-to-day-responsibilities",
      },
      {
        text: "Skills Required",
        link: "/lectures/data-science/foundations/skills-required",
      },
      {
        text: "Tools Used",
        link: "/lectures/data-science/foundations/tools-used",
      },
      {
        text: "Challenges Faced",
        link: "/lectures/data-science/foundations/challenges-faced",
      },
      {
        text: "Career Path",
        link: "/lectures/data-science/foundations/career-path",
      },
    ],
  },
  {
    text: "What is CRISP-DM?",
    items: [
      {
        text: "Overview",
        link: "/lectures/data-science/foundations/crisp-dm/",
      },
      {
        text: "Phases",
        link: "/lectures/data-science/foundations/crisp-dm/phases",
      },
      {
        text: "Example",
        link: "/lectures/data-science/foundations/crisp-dm/example",
      },
      {
        text: "Benefits and Limitations",
        link: "/lectures/data-science/foundations/crisp-dm/benefits-and-limitations",
      },
    ],
  },
]
