...
<template>
  <div class="w-full overflow-hidden">
    <div class="flex flex-row">
      <img
        class="h-40 w-40 md:w-48 lg:w-64 object-cover bg-emerald-500"
        :src="img_url"
        :alt="title"
      />
      <div
        class="flex flex-col p-4 border-t border-r border-b border-slate-400 dark:border-slate-700 border-dashed"
      >
        <div class="flex flex-col gap-2 items-start">
          <div class="uppercase tracking-wide text-sm text-emerald-500 font-semibold">
            {{ title }}
          </div>
          <div class="text-slate-500 line-clamp-3 text-sm">{{ description }}</div>
        </div>
